package com.coolteam.igame.config;

public final class StaticResourcesConfig {
    public final static int STAGE_WIDTH = 368;
    public final static int STAGE_HEIGHT = 235;

    public final static String LOGIN_VIEW_PATH = "LoginView.fxml";
    public final static String MAIN_VIEW_PATH = "HomeView.fxml";

    public final static String RDS_ENDPOINT = "jupiter-database.cx1llbdsmoon.us-east-1.rds.amazonaws.com";
    public final static String DBNAME = "igamedb";
    public final static String USERNAME = "admin";
    public final static String PASSWORD = "qwerasdf";
    public final static String NO_EXISTING_USER = "__NO__EXISTING__USER__";
    public final static String WRONG_PASSWORD = "__WRONG_PASSWORD__";

    public final static String exp = "src/main/resources/music/exp-1688.wav";
    public final static String d = "src/main/resources/music/6199.mp3";

}

